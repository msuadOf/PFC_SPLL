                                     ...
                                     float32_t acValue)
{
    ...
    spll_obj->sine=(float32_t)sin(spll_obj->theta);
    spll_obj->cosine=(float32_t)cos(spll_obj->theta);
}
...
void spll_Outputs_wrapper(const float32_t *acValue,
                          float32_t *theta,
                          float32_t *sinf)
{
/* Output_BEGIN */
SPLL_1PH_SOGI_run(&spll1,(float32_t)*acValue);
    *sinf = spll1.sine;
    *theta=spll1.theta;
/* Output_END */
}
